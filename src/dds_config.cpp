...
void DDS::setChipSelect(bool cs) {
  ...
  //uint32_t val = 0;
  //if (cs) { val = 1;}
  //fprintf(stderr, "Chip Select set to: %d\n", cs);
  // fprintf(stderr,"Pre set CS:            0x%08x\n",AXI->reg[0]);
  //fprintf(stderr, "What reg[0] should be: 0x%08x\n",axiCopy | (cs << 24));
  AXI->reg[0] = axiCopy | (cs << 24);
  chipSelect = cs;
  //fprintf(stderr, "what reg[0] is:        0x%08x\n",AXI->reg[0]);
}
/*void
 *getConfigData()
 *
...
uint64_t DDS::getProfileCMD(bool cs){
  ...
  //fprintf(stderr, "ftw: 0x%08x\n",ftw);
  //fprintf(stderr, "pow: 0x%04x\n",pow);
  //fprintf(stderr, "profile word: 0x%016llx\n",profData);
  //uint32_t profDataMSB = (profData & 0xFFFFFFFF00000000) >> 32;
  //uint32_t profDataLSB = (profData & 0x00000000FFFFFFFF);
  // fprintf(stderr, "ftw: 0x%08x\n",profDataLSB);
  //fprintf(stderr, "pow: 0x%08x\n",profDataMSB);

  //if (profDataLSB == 0) {profDataLSB = 1;}
  //DDSWrite(0x0E,profDataMSB, profDataLSB);
  return profData;
}
...
void DDS::ResetDDS(){
  ...
  //fprintf(stderr, "Reset AXI: 0x%08x\n", AXI->reg[0]);
 ...
 */
void DDS::IOReset(){
  // uint32_t otherBits = AXI->reg[0] &
  AXI->reg[0] = 0x00010000; //Flip the IOreset bit high
  //fprintf(stderr,"IOReset Axi: 0x%08x\n",AXI->reg[0]);
